...
function TransBtn({ onClick, title, active, children }) {
    return _jsx("button", { onClick: onClick, title: title, className: `w-7 h-7 rounded flex items-center justify-center transition-all duration-150 ${active ? "bg-sky-600 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800 active:scale-95"}`, children: children });
}
export function Timeline() {
    ...
    const prevPhase = useCallback(() => { if (phaseIdx > 0)
        jumpToPhase(phases[phaseIdx - 1].id); }, [phaseIdx, phases, jumpToPhase]);
    const nextPhase = useCallback(() => { if (phaseIdx < phases.length - 1)
        jumpToPhase(phases[phaseIdx + 1].id); }, [phaseIdx, phases, jumpToPhase]);
    // Keyboard
    useEffect(() => {
        const h = (e) => {
    ...
    return (_jsxs("div", { className: "flex-shrink-0 bg-slate-900/95 border-t border-slate-800 select-none", style: { height: 64 }, children: [_jsxs("div", { className: "h-7 flex items-center gap-1.5 px-3", children: [_jsx(TransBtn, { onClick: stop, title: t("ctrl.stop"), children: _jsx("svg", { width: "11", height: "11", viewBox: "0 0 12 12", children: _jsx("rect", { x: "1", y: "1", width: "10", height: "10", rx: "1", fill: "currentColor" }) }) }), _jsx(TransBtn, { onClick: skipBack, title: t("ctrl.skipBack"), children: _jsxs("svg", { width: "12", height: "12", viewBox: "0 0 12 12", children: [_jsx("path", { d: "M7 2L3 6l4 4V2z", stroke: "currentColor", strokeWidth: "1.5", fill: "none" }), _jsx("path", { d: "M10 2L8 6l2 4V2z", stroke: "currentColor", strokeWidth: "1.5", fill: "none" })] }) }), _jsx(TransBtn, { onClick: stepBackward, title: t("ctrl.prevFrame"), children: _jsx("svg", { width: "11", height: "11", viewBox: "0 0 12 12", children: _jsx("path", { d: "M9 2L4 6l5 4V2zM3 2v8", stroke: "currentColor", strokeWidth: "1.5", fill: "none" }) }) }), _jsx("button", { onClick: playing ? pause : play, className: "w-8 h-7 rounded-md bg-sky-600 hover:bg-sky-500 text-white flex items-center justify-center transition-all duration-150 active:scale-95", title: playing ? t("ctrl.pause") : t("ctrl.play"), children: playing
                        ...
                        return _jsxs("button", { onClick: (e) => { e.stopPropagation(); jumpToPhase(p.id); }, className: "absolute -translate-x-1/2 group", style: { left: `${l}%`, top: 0 }, title: t(p.label), children: [_jsx("div", { className: "rounded-full transition-all duration-200", style: { width: isActive ? 14 : 10, height: isActive ? 14 : 10, marginTop: isActive ? 0 : 2, backgroundColor: isCurrentOrPast ? c : "#334155", border: isActive ? "2px solid white" : isPast ? `2px solid ${c}` : "2px solid #475569", boxShadow: isActive ? `0 0 8px ${c}` : "none" } }), _jsx("span", { className: `absolute top-full mt-1 left-1/2 -translate-x-1/2 text-[9px] whitespace-nowrap transition-all duration-200 ${isActive ? "text-white font-medium opacity-100" : isPast ? "text-slate-400 opacity-80" : "text-slate-600 opacity-0 group-hover:opacity-60"}`, children: t(p.label) })] }, p.id);
                    }), _jsxs("div", { className: "absolute top-[-2px] -translate-x-1/2 z-10", style: { left: `${progress}%` }, children: [_jsx("div", { className: "w-0.5 h-5 bg-sky-400 rounded-full shadow-sm shadow-sky-500/30" }), _jsx("div", { onMouseDown: (e) => { e.stopPropagation(); e.preventDefault(); dragging.current = true; const m = (ev) => { if (dragging.current)
