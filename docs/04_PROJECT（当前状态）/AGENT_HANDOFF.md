...
- **S88+（2D 动画系统，下一主线）**：S88-A 已闭环；S88-B1 第一阶段完成（isPlaneScene 判定 + 自由落体默认 2D 路由 + 2D 原生 PhaseCard/FormulaStrip），公共 2D 原语抽取待下一阶段；B2 三场景（projectile/inclined_plane/ohms_law）布局规格已交付待实现。
