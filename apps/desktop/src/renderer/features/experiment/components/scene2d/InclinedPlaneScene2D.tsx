...
export function InclinedPlaneScene2D() {
  ...
  // slide distance measured down the incline, clamped to the incline length
  const s = Math.min(0.5 * a * currentTime * currentTime, L);
  const bx = (L - s) * Math.cos(rad);
  const by = (L - s) * Math.sin(rad);

  const { toSvg } = useMemo(() => {
    const world: Point2[] = [
      { x: 0, y: 0 },
      { x: L * Math.cos(rad), y: 0 },
      { x: L * Math.cos(rad), y: L * Math.sin(rad) },
      { x: L * Math.cos(rad) + 0.5, y: L * Math.sin(rad) + 0.5 },
      { x: -0.5, y: 0 },
    ];
  ...
  const baseRight = toSvg({ x: L * Math.cos(rad), y: 0 });
  const tip = toSvg({ x: L * Math.cos(rad), y: L * Math.sin(rad) });
  const blockSvg = toSvg({ x: bx, y: by });
  ...
  return (
    <div className="w-full h-full" style={{ background: "#0A0E1A" }}>
      <svg width="100%" height="100%" viewBox="0 0 1200 675" preserveAspectRatio="xMidYMid meet">
        {/* incline triangle (right angle bottom-right, slope descends to lower-left) */}
        ...
        {/* block, rotated to lie on the slope (down-left direction) */}
        <g transform={`translate(${blockSvg.x},${blockSvg.y}) rotate(${-angleDeg})`}>
          <rect x={-18} y={-12} width={36} height={24} rx={4} fill={COLORS.structure} stroke={COLORS.emphasis} strokeWidth={2} />
        </g>

        {/* forces: G vertically down; v along the slope downhill (screen: -cosθ, +sinθ) */}
        <Arrow x={blockSvg.x} y={blockSvg.y - 12} length={forceLen} direction={{ x: 0, y: 1 }} color={COLORS.force} label="G" />
        {speedLen > 1 && (
          <Arrow x={blockSvg.x} y={blockSvg.y} length={speedLen} direction={{ x: -Math.cos(rad), y: Math.sin(rad) }} color={COLORS.velocity} label="v" labelItalic />
        )}
