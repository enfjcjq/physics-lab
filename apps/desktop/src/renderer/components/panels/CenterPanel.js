...
export function CenterPanel() {
    ...
    return (_jsxs("div", { className: "flex-1 relative min-w-0", children: [_jsx("div", { className: "absolute top-3 left-4 z-20", children: _jsx("div", { className: "flex gap-1 bg-slate-900/90 backdrop-blur-md border border-slate-600/50 rounded-xl p-1 shadow-xl", children: pluginRegistry.list().map((p) => (_jsxs("button", { onClick: async () => { if (p.id !== activePluginId)
                            await setActivePlugin(p.id); }, className: `px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 flex items-center gap-1 ${p.id === activePluginId
                            ? "bg-sky-600 text-white shadow-md shadow-sky-900/40"
                            : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/50"}`, title: t(p.name), children: [_jsx("span", { className: "text-xs", children: expIcons[p.id] || "⚡" }), _jsx("span", { className: "inline", children: t(p.name) })] }, p.id))) }) }), pluginLoading && (_jsx("div", { className: "absolute inset-0 z-20 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm", children: _jsxs("div", { className: "flex flex-col items-center gap-4", children: [_jsx("div", { className: "w-64 h-40 bg-slate-800/80 rounded-xl animate-pulse border border-slate-700/30 flex items-center justify-center", children: _jsx("div", { className: "w-16 h-16 rounded-full bg-slate-700/50" }) }), _jsx("div", { className: "w-48 h-2 bg-slate-800 rounded-full overflow-hidden", children: _jsx("div", { className: "h-full bg-gradient-to-r from-sky-600 to-violet-600 rounded-full animate-pulse", style: { width: "60%" } }) }), _jsx("span", { className: "text-xs text-slate-500", children: t("ui.loading") })] }) })), _jsx(Scene3D, {}), _jsx(TeachingOverlay, {}), _jsx(FormulaHTML, {}), _jsx("div", { className: "absolute bottom-4 left-4 z-10", children: _jsxs("div", { className: "bg-slate-900/80 backdrop-blur border border-slate-700/50 rounded-lg px-3 py-1.5 flex items-center gap-2", children: [_jsx("span", { className: "w-2 h-2 rounded-full bg-sky-400" }), _jsx("span", { className: "text-xs text-slate-300", children: currentPhase ? t(currentPhase.label) : currentPhaseId })] }) }), _jsx("div", { className: "absolute top-3 left-1/2 -translate-x-1/2 z-10", children: _jsx("div", { className: "flex items-center gap-0 bg-slate-900/80 backdrop-blur border border-slate-700/50 rounded-full px-1 py-1 shadow-lg", children: phases.map((p, i) => (_jsxs("div", { className: "flex items-center", children: [_jsxs("button", { onClick: () => jumpToPhase(p.id), className: `flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-all duration-300 ${currentPhaseId === p.id
                                    ? "bg-sky-600 text-white shadow-md shadow-sky-900/30 scale-105"
                                    : "text-slate-500 hover:text-slate-300 hover:bg-slate-800/50"}`, title: p.description || t(p.label), children: [_jsx("span", { className: `w-1.5 h-1.5 rounded-full ${currentPhaseId === p.id ? "bg-white" : "bg-slate-600"}` }), _jsx("span", { className: "inline", children: t(p.label) })] }), i < phases.length - 1 && (_jsx("div", { className: "w-3 h-px bg-slate-700" }))] }, p.id))) }) })] }));
}
